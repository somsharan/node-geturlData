...
*/
	function parseMetaData($){
		for(var key in $('meta')){
		//Object.keys($('meta')).map(function(value, index) {
			...
			if(key == $('meta').length - 1){
				parseBody($)
				//console.log(resObj,'mappedDatamappedData');
			}
...
*/
	function parseBody($){
		...
		for(var i = 0; i < $('img').length; i++){
			...
			if($('img').length < 10 && i == $('img').length -1) {console.log('came her e beforeand exit')
				callback(null, resObj);
				return;
			} else if(i == 9){console.log('came her and exit')
				callback(null, resObj);
				//console.log('came her and exit')
				return;
			}
		}
	}
